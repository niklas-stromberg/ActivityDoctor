...
abstract class Dialog_Statistics {
	...
	public static Dialog getDialog(final Context c, int since_boot) {
		...
		int thisWeek = db.getMinutes(date.getTimeInMillis(), System.currentTimeMillis()) + since_boot;

		date.setTimeInMillis(Util.getToday());
		date.set(Calendar.DAY_OF_MONTH, 1);
		int thisMonth = db.getMinutes(date.getTimeInMillis(), System.currentTimeMillis()) + since_boot;
